...
async function page(route,context){const {targetId}=await browser.send('Target.createTarget',{url:'about:blank',browserContextId:context});const targets=await (await fetch(`http://127.0.0.1:${port}/json/list`)).json(),p=new CDP(targets.find(t=>t.id===targetId).webSocketDebuggerUrl);
  await p.send('Runtime.enable');await p.send('Page.enable');await p.send('Network.enable');await p.send('Emulation.setDeviceMetricsOverride',{width:1440,height:1100,deviceScaleFactor:1,mobile:false});await p.send('Page.navigate',{url:base+(route==='/'?'/play':route)});await until(()=>p.eval(`document.readyState==='complete'`));if(await p.eval(`Boolean(document.querySelector('#guest-enter'))`)){await until(()=>p.eval(`!document.querySelector('#guest-enter').disabled`));await p.click('#guest-enter');}await until(()=>p.eval(`document.readyState==='complete'&&typeof TTRSPreferences!=='undefined'`));return p;
}
